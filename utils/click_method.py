...
def get_next_click3D_torch_2(prev_seg, gt_semantic_seg):
    """
    是从前一次分割结果 prev_seg 和真实标签 gt_semantic_seg 中，选择合适的位置作为用户点击点（提示），
    并返回这些点击点及其标签。具体来说，函数通过检查 假阴性（False Negatives, FN）和 假阳性（False Positives, FP）区域来确定哪些区域
    是用户可能感兴趣的，然后随机选择这些区域中的点作为点击。
    """
    ...
    for i in range(gt_semantic_seg.shape[0]):
        ...
        # import pdb; pdb.set_trace()
        if fn_masks[i, 0, point[1], point[2], point[3]]:  # 如果选中的点来自假阴性区域，则认为它是一个正样本（目标区域），标签为 True
            is_positive = True
        else:  # 如果选中的点来自假阳性区域，则认为它是一个负样本（背景区域），标签为 False
            is_positive = False
