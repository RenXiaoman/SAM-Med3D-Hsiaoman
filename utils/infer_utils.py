...
def sam_model_infer(model,
                    roi_image,
                    roi_gt=None,  # 推理的时候不为None
                    prompt_generator=random_sample_next_click,
                    prev_low_res_mask=None,  # 推理的时候为None
    ...
    # 2. 检查真实标签是否为空
    # 如果标签全为零，并且需要点击来进行交互式分割，那么模型的预测结果可能会为空，函数会返回一个全零的掩膜，表示没有预测结果。
    ...
    with torch.no_grad():
        ...
        image_embeddings = model.image_encoder(input_tensor)  # 通过图像编码器生成图像嵌入 shape is [1, 384, 8, 8, 8]
        ...
        if prev_low_res_mask is None: # Initialize low_res_mask for the decoder
            # 表示没有低分辨率掩膜，则通过将图像尺寸缩小四分之一来初始化一个低分辨率的掩膜
             prev_low_res_mask = torch.zeros(1, 1, roi_image.shape[2] // 4,  # [1, 1, 32, 32, 32]
                                             roi_image.shape[3] // 4,
        ...
        for _ in range(num_clicks):
            if roi_gt is not None:
                ...
                if points_coords.shape[1] == 0: # Only for the very first click if no GT
                    ...
                    break 
            '''
                new_points_co shape is [1, 1, 3], value is [[[68, 71, 72]]]
                new_points_la shape is [1, 1], value is [[1]]
                points_coords shape is [1, 0, 3], value is []
                points_labels shape is [1, 0], value is []
            '''

            print(' ==== ')

            # 8. 更新点击点和标签 points_coords的shape原来为[1, 0, 3], points_labels的shape为[1, 0]
            points_coords = torch.cat([points_coords, new_points_co], dim=1)
            points_labels = torch.cat([points_labels, new_points_la], dim=1)

            '''
                points_coords shape is [1, 1, 3], value is [[[68., 71., 72.]]]
                points_labels shape is [1, 1], value is [[1.]]
            '''
...
def data_preprocess(subject, meta_info, category_index, target_spacing, crop_size=128):
    ...
    meta_info["original_subject_spatial_shape"] = subject.image.spatial_shape # D, H, W  (384, 384, 19)


    # step-1: resample online
    # 打印没有采样前的image和label的spacing和shape
    # print("The original image spacing: ", subject.image.spatial_shape, subject.image.spacing)
    # print("The original label spacing: ", subject.label.spatial_shape, subject.label.spacing)
    # resampler = tio.Resample(target=target_spacing)
    # subject_resampled = resampler(subject)

    # 打印采样后的image和label的spacing和shape
    # print("The resampled image spacing: ", subject_resampled.image.spatial_shape, subject_resampled.image.spacing)
    # print("The resampled label spacing: ", subject_resampled.label.spatial_shape, subject_resampled.label.spacing)

    # assert subject_resampled.label.spatial_shape == subject_resampled.image.spatial_shape, "The shape of resampled input image and label is not equal!"

    # step-2: canonicalize  将3D医学图像数据重新定位为标准的 RAS+ 定向
    transform_canonical = tio.ToCanonical()
    subject_canonical = transform_canonical(subject)
    ...
    return roi_image, roi_label, meta_info  # ([1, 1, 128, 128, 128])
...
def validate_paired_img_gt(model, img_path, gt_path, output_path, num_clicks=1, crop_size=128, target_spacing=(1.5, 1.5, 1.5), seed=233):
    ...
    # 从标注文件中提取所有前景类别标签（如肿瘤、器官等）。
    # 初始化一个全零的 final_pred_numpy_original_grid，用于存储最终预测结果。
    exist_categories, final_pred_numpy_original_grid = get_category_list_and_zero_mask(gt_path)
    # exist_categories = 2, final_pred_numpy_original_grid.shape = (19, 384, 384) 且全0矩阵
    ...
    """
    subject: Subject(Keys: ('image', 'label'); images: 2)
    meta_info: {
        'sitk_image_object': <SimpleITK.SimpleITK.Image; proxy of <Swig Object of type 'itk::simple::Image *' at 0x76d998143f90> >, 
        'sitk_origin': (-84.256508846899, -64.567159486491, 2.5595945183903), 
        'sitk_direction': (1.0, 2.0510340000000023e-10, 1.671085560165089e-17, -2.0024163387483253e-10, 0.9762960061020008, 
               0.2164396185297002, 4.439248534040297e-11, -0.2164396185297002, 0.9762960061020008), 
        'sitk_spacing': (0.5, 0.5, 2.999999996213984), 
        'original_numpy_shape': (19, 384, 384)
        }
    """
    # fig = plt.figure()
    # matshow3d(np.array(subject.image), fig=fig, title="3D ROI Image, before resampling", cmap='gray')
    # plt.show()
    for category_index in exist_categories:
        ...
        # fig = plt.figure()
        # matshow3d(roi_image, fig=fig, title="3D ROI Image", cmap='gray', channel_dim=1)
        # plt.show()
        #
        # fig = plt.figure()
        # matshow3d(roi_label, fig=fig, title="3D ROI Label", cmap='gray', channel_dim=1)
        # plt.show()
